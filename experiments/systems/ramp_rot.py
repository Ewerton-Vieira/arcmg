...
class Ramp_rot(BaseSystem):
    ...
    # def f(self,s):
    #     return self.f_(self.f_(self.f_(self.f_(self.f_(self.f_(self.f_(self.f_(self.f_(self.f_(self.f_(self.f_(s))))))))))))
    ...
    def transform(self,x):
        "clockwise rotation"
        return np.matmul(x, self.R(2*np.pi*np.linalg.norm(x)))
        # x=np.array(x)
    ...
    def which_attracting_region(self, s):
        ...
        if np.linalg.norm(s - np.array(self.attractors()[1]))<0.5:
            return 1
        elif np.linalg.norm(s - np.array(self.attractors()[0]))<0.5:
            return 0
        else:
            return False
